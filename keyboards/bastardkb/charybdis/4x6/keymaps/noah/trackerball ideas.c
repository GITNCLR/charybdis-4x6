...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        case VOLMODE:
            ...
            if (!volmode_active) { vol_acc = 0; vol_last_dir = 0; }
            return false; // pure hold
    }
    return true;
}

// --- Small helpers ---
static inline report_mouse_t freeze_mouse(report_mouse_t r) { return (report_mouse_t){0}; }

static inline void caret_send_arrow(uint16_t keycode) {
    if (get_mods() & MOD_MASK_SHIFT) tap_code16(S(keycode));
...
static inline int32_t caret_emit_many(int32_t delta, uint16_t positive, uint16_t negative, uint16_t threshold) {
    while (delta >= threshold)               { caret_send_arrow(positive); delta -= threshold; }
    while (delta <= -(int32_t)threshold)     { caret_send_arrow(negative); delta += threshold; }
    return delta;
}
...
static inline void caret_update_dominant_axis(int16_t dx, int16_t dy) {
    ...
    if (ax > ay && ax > 0) dominant_axis = 'x';
    else if (ay > ax && ay > 0) dominant_axis = 'y';
}
...
static inline report_mouse_t handle_volume_mode(report_mouse_t mouse_report) {
    ...
    if (dy != 0) {
        ...
        vol_acc += dy;
        while (vol_acc >= VOLUME_THRESHOLD)  { tap_code(KC_AUDIO_VOLU); vol_acc -= VOLUME_THRESHOLD; }
        while (vol_acc <= -VOLUME_THRESHOLD) { tap_code(KC_AUDIO_VOLD); vol_acc += VOLUME_THRESHOLD; }
    }
...
static inline report_mouse_t handle_caret_mode(report_mouse_t mouse_report) {
    ...
    if (dominant_axis == 'x') {
        ...
        if (dx != 0) { acc_x = 0; last_x_dir = 0; }
    }

    return freeze_mouse(mouse_report);
}

// --- Main hook (early exits, clear priority) ---
